...
def find_quexian(img,accurate):
    ...
    PATH_TO_LABELS = os.path.join(CWD_PATH,'find_quexian_faster_rcnn_model','labelmap.pbtxt')
    ...
    for i in range(min(max_boxes_to_draw, box.shape[0])):
        if scores[i] > min_score_thresh:
            ...
            print(xmin, ymin, xmax, ymax,scores[i])
            boxforuse = [xmin,ymin,xmax,ymax]
            box_all.append(boxforuse)
            print(box_all)
    return box_all
